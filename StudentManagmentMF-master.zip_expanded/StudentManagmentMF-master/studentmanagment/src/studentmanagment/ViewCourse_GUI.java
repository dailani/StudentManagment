...
public class ViewCourse_GUI {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ViewCourse_GUI window = new ViewCourse_GUI();
	...
	private void initialize() {
		...
		List<StudentMF> viewStudentList = StudentDataOper.viewCourse(Main_GUI.oper[Main_GUI.list.getSelectedIndex()]);
		...
		for(StudentMF s : viewStudentList ) {
			...
			data[row][6]= s.getDate();
			data[row][7] = String.valueOf(s.getDue());
			row++;		
		}
		String columnNames[] = {"No." ,"FirstName", "LastName", "PhoneNo" , "Course" , "Fee", "Date","Due"};
